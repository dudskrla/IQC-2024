...
### below is added functions



def alpha_grid_time_horizon(s, alpha_id):
    ...
    if alpha_id in list(df.alpha_id):
        alpha = df[df.alpha_id == alpha_id].iloc[0]
        regular = alpha.regular
        neut = alpha.neutralization
    else:
        alpha = get_simulation_result_json(s, alpha_id)
        regular = alpha['simulate_data']['regular']
        neut = alpha['simulate_data']['settings']['neutralization']
    ...
    if alpha.turnover < 0.2:
        time_horizons = [252,504,756]
    elif alpha.turnover < 0.4:
        time_horizons = [20,63,252,504,756]
    else:
        time_horizons = [20,63]
    ...
    regular_lines = regular.split('\n')

    

    expression_list = ['\n'.join([line[:line.index(',126')]+f',{time_horizon}'+line[line.index(',126')+4:] if ',126' in line else line for line in regular_lines])\
    for time_horizon in time_horizons]


    res_t = [expression_list, [neut]*len(expression_list), [0]*len(expression_list),[alpha_id]*len(expression_list),[alpha_id]*len(expression_list)]
    
    
    return [list(x) for x in zip(*res_t)] # return expression(0) and neutralization option(1), decay(2), mother_alpha(3), grandmother_alpha(4)



def alpha_grid_decrease_turnover(s, alpha_id):
    df = pd.concat(sum([[pd.read_csv(f'./{layer_name}_2/'+x).drop('Unnamed: 0',axis =1) for x in os.listdir(f'./{layer_name}_2/')] for layer_name in ['layer1','layer2','layer3']],[])).reset_index(drop=True)
    if alpha_id in list(df.alpha_id):
        alpha = df[df.alpha_id == alpha_id].iloc[0]
        regular = alpha.regular
    ...
    if alpha.turnover > 1:
        decays = [2,4,6]
    elif alpha.turnover > 1:
        decays = [2,4]
    else:
        return []     
    
    
    original_last_line = [f'high_turnover_alpha = '+regular.split('\n')[-1]+';']

    event = 'volume>adv20'

    last_line = [f'event = {event};',f'trade_when(event,high_turnover_alpha,-1)']

    tw_expression = '\n'.join(regular.split('\n')[:-1] + original_last_line + last_line)
    

    res_t = [[tw_expression]+[regular]*len(decays), [neut]*(1+len(decays)), [0]+decays, [alpha_id]*(1+len(decays)),[alpha_id]*(1+len(decays))]
    
    return [list(x) for x in zip(*res_t)] # return expression(0) and neutralization option(1), decay(2), mother_alpha(3), grandmother_alpha(4)



def generate_selection(s,selection_expression:str, delay: int = 1, region:str = "USA", limit : int = 10, selection_handling: str = "NON_NAN",  ):

    selection = s.get(
        url = f"https://api.worldquantbrain.com/simulations/super-selection?settings.instrumentType=EQUITY&settings.region={region}&settings.delay={delay}&selection={urllib.parse.quote(selection_expression)}&limit={limit}&selectionLimit={limit}&selectionHandling={selection_handling}"
    )
...
def generate_super_alpha(
    ...
    pasteurization: str = "ON",
    visualization: bool = False,
    selectionHandling: str = "NON_ZERO",
...
def simulate_alpha_multi(
    ...
    with ThreadPool(limit_of_concurrent_simulations) as pool:
        ...
        result = pool.map(partial(simulate_single_alpha, s), tasks)
    
    
    simulation_config=DEFAULT_CONFIG
    func = lambda x: get_specified_alpha_stats(s, x['alpha_id'], x['simulate_data'], **simulation_config)
    with ThreadPool(3) as pool:
        result2 =  pool.map(func, result)

            
    return result2 


def get_max_prod_corr(s, alpha_id) -> float:
    """
    Function gets alpha's prod correlation
    and return max prod corr value to dataframe and the number of alphas over 0.7
    """
    ...
    return [[x[1] for x in result.json()['records'] if x[2] != 0][-1], sum([x[2] for x in result.json()['records'] if x[1] > 0.7 ])]



def get_max_prod_corr_multi(
    s,
    alpha_id_list,
    ...
    with ThreadPool(limit_of_concurrent_simulations) as pool:
        ...
        result = pool.map(partial(get_max_prod_corr, s), tasks)
